...
import { Submission, WeeklyTask } from '../shared/schema';
...
function submission(milestone_type: Submission['milestone_type'], status: Submission['status']): Submission {
  return { id: milestone_type, weekly_task_id: task.id, milestone_type, status } as Submission;
}
...
describe('accountability rules', () => {
  ...
  it('returns only newly earned achievements', () => {
    const result = missingAchievementTypes(
      [{ type: 'first_submission' } as any],
      {
