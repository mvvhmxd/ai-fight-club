...
export async function verifyGitHubRepository(
  ...
  if (!repository) return { valid: false, error: 'Enter a valid public GitHub repository URL' };
  ...
  try {
    ...
    if (!repoResponse.ok) return { valid: false, error: 'GitHub repository was not found or is not public' };

    const commitsResponse = await request(`${base}/commits?per_page=1`, { headers });
    if (!commitsResponse.ok) return { valid: false, error: 'GitHub repository must contain at least one commit' };
