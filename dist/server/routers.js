...
async function topicIsUnlocked(userId, topicId) {
    ...
    const ordered = [...topics].sort((a, b) => (stageOrder.get(a.stage_id) || 0) - (stageOrder.get(b.stage_id) || 0) ||
...
router.get('/stages/:stageId/topics', async (req, res) => {
    try {
...
router.post('/submissions', async (req, res) => {
    try {
        ...
        const requiredMilestones = Array.isArray(task.required_milestones)
            ...
            : JSON.parse(task.required_milestones);
...
// ============ EXCUSE ENDPOINTS ============
router.post('/submissions/:submissionId/excuse', requireAdmin, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
...
router.get('/admin/overdue', requireAdmin, async (req, res) => {
    try {
...
router.get('/admin/curriculum', requireAdmin, async (_req, res) => {
    try {
...
router.post('/admin/stages', requireAdmin, async (req, res) => {
    try {
...
router.put('/admin/stages/:stageId', requireAdmin, async (req, res) => {
    try {
        const { name, order_index, description = '' } = req.body;
        if (!name?.trim() || !positiveInteger(order_index)) {
            return res.status(400).json({ error: 'Name and a positive order are required' });
...
router.put('/admin/topics/:topicId', requireAdmin, async (req, res) => {
    try {
        const { stage_id, name, order_index, resources = {} } = req.body;
        if (!stage_id || !name?.trim() || !positiveInteger(order_index)) {
            return res.status(400).json({ error: 'Stage, name, and a positive order are required' });
...
function validMilestones(value) {
...
router.put('/admin/tasks/:taskId', requireAdmin, async (req, res) => {
    try {
        const { topic_id, week_number, assigned_date, due_date, required_milestones } = req.body;
        if (!topic_id || !positiveInteger(week_number) || !assigned_date || !due_date ||
            !validMilestones(required_milestones)) {
...
router.put('/admin/reorder/stages', requireAdmin, async (req, res) => {
    try {
...
router.put('/admin/reorder/topics', requireAdmin, async (req, res) => {
    try {
        const { stage_id, ids } = req.body;
        if (!stage_id || !Array.isArray(ids) || ids.some((id) => typeof id !== 'string')) {
...
router.put('/admin/reorder/tasks', requireAdmin, async (req, res) => {
    try {
        const { topic_id, ids } = req.body;
        if (!topic_id || !Array.isArray(ids) || ids.some((id) => typeof id !== 'string')) {
...
router.delete('/admin/stages/:stageId', requireAdmin, async (req, res) => {
    try {
...
router.delete('/admin/topics/:topicId', requireAdmin, async (req, res) => {
    try {
...
router.delete('/admin/tasks/:taskId', requireAdmin, async (req, res) => {
    try {
...
async function updateProgress(userId, weeklyTaskId) {
    ...
    if (task && streak && isTaskComplete(task, submissions)) {
        const result = nextStreak(streak.current_streak_weeks, streak.longest_streak_weeks, streak.last_complete_week, new Date());
        if (result.changed) {
